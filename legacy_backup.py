...
try:
    ...
    print("💡 Tente: pip install opencv-python numpy ultralytics")
    input("Pressione Enter para sair...")
    exit()

# Configurações dos modelos (do legacy)
MODELOS = {
...
class SimpleStateManager:
    ...
    def __init__(self, config):
        ...
        self.PERFIL_CAIXA = config['perfil_caixa']
        ...
        self.buffer_roi = deque(maxlen=config['tamanho_buffer_estabilizacao'])
        self.buffer_contagem_itens = deque(maxlen=config['tamanho_buffer_estabilizacao'])
        self.buffer_divisor_presente = deque(maxlen=config['tamanho_buffer_estabilizacao'])
...
class YOLODetector:
    """Detector usando modelos YOLO do legacy"""
    ...
    def __init__(self):
        ...
        try:
            ...
            roi_path = MODELOS['roi_detector']
            item_path = MODELOS['item_detector']
            
            if not os.path.exists(roi_path):
                raise FileNotFoundError(f"❌ Modelo ROI não encontrado: {roi_path}")
            if not os.path.exists(item_path):
                raise FileNotFoundError(f"❌ Modelo de itens não encontrado: {item_path}")
            
            # Carregar modelos
            print(f"📦 Carregando modelo ROI: {roi_path}")
            self.roi_model = YOLO(roi_path)
            
            print(f"📦 Carregando modelo de itens: {item_path}")
            self.item_model = YOLO(item_path)
            
            print("✅ Modelos YOLO carregados com sucesso!")
            print(f"🎯 Confiança ROI/Itens: {CONFIDENCIA_LIMITE}")
            print(f"🎯 Confiança Divisores: {CONFIDENCIA_DIVISOR}")
    ...
    def detectar_objetos(self, frame):
        """Detecta ROI, itens e divisores usando modelos YOLO"""
        try:
            # 1. Detectar ROI (caixas)
            deteccoes_roi = self.roi_model.predict(source=frame, conf=CONFIDENCIA_LIMITE, verbose=False)[0]
            caixas_detectadas = []
            
            for box in deteccoes_roi.boxes:
                coords = list(map(int, box.xyxy[0].tolist()))
                confianca = float(box.conf[0])
                caixas_detectadas.append((coords, confianca))
            
            # 2. Detectar Itens (confiança padrão)
            ...
            for box in deteccoes_itens.boxes:
                if int(box.cls) == 0:  # Classe 0 = item
                    coords = list(map(int, box.xyxy[0].tolist()))
                    confianca = float(box.conf[0])
                    itens_detectados.append((coords, confianca))
            
            # 3. Detectar Divisores (confiança baixa)
            ...
            for box in deteccoes_divisores.boxes:
                if int(box.cls) == 1:  # Classe 1 = divisor
                    coords = list(map(int, box.xyxy[0].tolist()))
                    confianca = float(box.conf[0])
                    divisores_detectados.append((coords, confianca))
            
            return caixas_detectadas, itens_detectados, divisores_detectados
            
        except Exception as e:
            print(f"❌ Erro na detecção: {e}")
            return [], [], []

def desenhar_deteccoes(frame, caixas, itens, divisores):
    """Desenha as detecções no frame com cores do legacy"""
    
    # Desenhar ROI (rosa) - apenas retângulo
    for coords, conf in caixas:
        x1, y1, x2, y2 = coords
        cv2.rectangle(frame, (x1, y1), (x2, y2), CORES_LEGACY['roi'], 3)
        cv2.putText(frame, f"ROI", (x1, y1 - 8), 
                   cv2.FONT_HERSHEY_SIMPLEX, 0.5, CORES_LEGACY['roi'], 2)
    
    # Desenhar Itens (verde) - apenas retângulos, sem números nem círculos
    for coords, conf in itens:
        x1, y1, x2, y2 = coords
        cv2.rectangle(frame, (x1, y1), (x2, y2), CORES_LEGACY['item'], 2)
    
    # Desenhar Divisores (amarelo) - apenas retângulos
    for coords, conf in divisores:
        x1, y1, x2, y2 = coords
        cv2.rectangle(frame, (x1, y1), (x2, y2), CORES_LEGACY['divisor'], 2)
        cv2.putText(frame, f"DIV", (x1, y1 - 8), 
                   cv2.FONT_HERSHEY_SIMPLEX, 0.3, CORES_LEGACY['divisor'], 1)

def filtrar_itens_na_roi(itens, caixas):
    """Filtra apenas itens que estão dentro da ROI (como no legacy)"""
    if not caixas:
        return []
...
def desenhar_painel_status(frame, status, width, height, contadores_yolo):
    """Desenha painel de status consolidado (mais alto, menos largo)"""
    # Painel mais alto e menos largo conforme solicitado
    panel_width = 250  # Aumentado de 200 para 250
    panel_height = 220  # Aumentado de 180 para 220
    ...
    # Status YOLO
    yolo_status = "YOLO: ON" if contadores_yolo['detection_enabled'] else "YOLO: OFF"
    cv2.putText(frame, yolo_status, (panel_x + 10, y_offset), 
               cv2.FONT_HERSHEY_SIMPLEX, 0.4, CORES_LEGACY['status'], 1)
    
    # Contadores YOLO
    y_offset += line_height + 2
    cv2.putText(frame, f"ROI: {contadores_yolo['roi']}", (panel_x + 10, y_offset), 
               cv2.FONT_HERSHEY_SIMPLEX, 0.35, CORES_LEGACY['roi'], 1)
    
    y_offset += line_height
    cv2.putText(frame, f"ITENS: {contadores_yolo['itens_roi']}", (panel_x + 10, y_offset), 
               cv2.FONT_HERSHEY_SIMPLEX, 0.35, CORES_LEGACY['item'], 1)
    
    y_offset += line_height
    cv2.putText(frame, f"DIVISORES: {contadores_yolo['divisores']}", (panel_x + 10, y_offset), 
               cv2.FONT_HERSHEY_SIMPLEX, 0.35, CORES_LEGACY['divisor'], 1)
    ...
    # Estado atual do StateManager
    y_offset += 15
    estado_display = status['estado'].replace('_', ' ')
    cv2.putText(frame, f"ESTADO:", (panel_x + 10, y_offset), 
               cv2.FONT_HERSHEY_SIMPLEX, 0.35, CORES_LEGACY['text'], 1)
    y_offset += 15
    cv2.putText(frame, estado_display, (panel_x + 10, y_offset), 
               cv2.FONT_HERSHEY_SIMPLEX, 0.35, CORES_LEGACY['status'], 1)
    
    # Camada atual
    y_offset += line_height + 2
    cv2.putText(frame, f"CAMADA: {status['camada_atual']}", (panel_x + 10, y_offset), 
               cv2.FONT_HERSHEY_SIMPLEX, 0.35, CORES_LEGACY['text'], 1)
    
    # Contagem atual
    y_offset += line_height
    cv2.putText(frame, f"CONTAGEM: {status['contagem_atual']}/{status['meta_camada']}", (panel_x + 10, y_offset), 
               cv2.FONT_HERSHEY_SIMPLEX, 0.35, CORES_LEGACY['item'], 1)
    
    # Total de itens
    y_offset += line_height
    cv2.putText(frame, f"TOTAL: {status['total_itens']}", (panel_x + 10, y_offset), 
               cv2.FONT_HERSHEY_SIMPLEX, 0.35, CORES_LEGACY['text'], 1)
    
    # Progresso por camada
    y_offset += line_height + 5
    cv2.putText(frame, "CAMADAS:", (panel_x + 10, y_offset), 
               cv2.FONT_HERSHEY_SIMPLEX, 0.3, CORES_LEGACY['text'], 1)
    
    for camada, contagem in status['camadas'].items():
        y_offset += 12
        cv2.putText(frame, f"  {camada}: {contagem}", (panel_x + 15, y_offset), 
                   cv2.FONT_HERSHEY_SIMPLEX, 0.3, CORES_LEGACY['text'], 1)

try:
    # Inicializar detector YOLO e StateManager
    detector = YOLODetector()
    state_manager = SimpleStateManager(STATE_CONFIG)
    ...
    while True:
        if not paused:
            ...
            if detection_enabled:
                ...
                itens = filtrar_itens_na_roi(itens, caixas)
                desenhar_deteccoes(frame, caixas, itens, divisores)
                
                # Atualizar StateManager
                roi_presente = len(caixas) > 0
                state_manager.atualizar_estado(roi_presente, itens, divisores)
            ...
            desenhar_painel_status(frame, status, width, height, contadores_yolo)
            
            # Controles na parte inferior
            cv2.putText(frame, "ESC=Sair SPACE=Pausar D=YOLO R=Reset", (10, height-20), 
                       cv2.FONT_HERSHEY_SIMPLEX, 0.4, CORES_LEGACY['text'], 1)
            
            if paused:
                cv2.putText(frame, "PAUSADO", (width//2-50, height//2), 
                           cv2.FONT_HERSHEY_SIMPLEX, 1, CORES_LEGACY['alert'], 2)
